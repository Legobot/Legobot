...
class timerFunc():
    ...
    def runIfNeeded(self):
        timeDiff = (datetime.datetime.now() - self.lastRun).total_seconds()
        if timeDiff > self.interval and self.interval != -1:
            # run
            self.func()
...
class legoBot():
    def __init__(self, host, port, nick, nickpass, chans,
                 logfunc="", hostpw="", defaultFunc=None,
                 defaultFuncChar=""):
        self.host = host
        self.hostpw = hostpw
        self.port = port
        ...
        self.addFunc(
            ...
            this default help value can be overridden with a custom function")

    def defaultHelp(self, msgObj):
        if not msgObj.arg1:
            funcsAvailable = ", ".join(sorted(self.func.keys()))
            return "Available functions: %s" % funcsAvailable
    ...
    def addTimerFunc(self, function, min="*", hour="*", day="*", month="*",
                     dow="*", sec="*"):
        self.timerFuncList.append(legoCron.Event(function, min, hour,
                                                 day, month, dow, sec))

    def addDefaultFunc(self, func, char):
        # adds a func to run whenever trigger char is seen as the
        # first character if we don't match any other function
    ...
    def addFunc(self, name, function, helpText=""):
        # name should be str, function should be a function object
        # (as in a function without the parens)
    ...
    def __listen(self):
        ...
        for timerFunc in self.timerFuncList:
            self.threadList.append(threading.Thread(target=timerDaemon,
                                                    args=(timerFunc,
                                                          self.threadQueue,
                                                          self.chans)))
        ...
        for randTimerFunc in self.randTimerFuncList:
            self.threadList.append(
                threading.Thread(target=randTimerDaemon,
                                 args=(randTimerFunc.func,
                                       self.threadQueue,
                                       randTimerFunc.randMin,
                                       randTimerFunc.randMax,
                                       self.chans)))
            self.threadList[-1].daemon = True
            self.threadList[-1].start()
        ...
        while True:
            if select.select([self.connection], [], [], 1.0)[0]:
                ...
                for line in temp:
                    ...
                    msg = Message(line)
                    self.threadList.append(
                        threading.Thread(target=msg.read,
                                         args=(self.host,
                                               self.func,
                                               self.nick,
                                               self.logfunc,
...
class Message():
    ...
    def read(self, host, func, nick, logfunc, threadQueue,
        ...
        if self.splitMessage[0][0:4] == "PING":
            ...
            try:
                ...
                self.actualUserName = self.\
                    splitMessage[0][1:self.splitMessage[0].find("!")].lower()
    ...
    def getReturnVal(self, func, nick):
        ...
        if isinstance(tempVal, tuple):
            ...
            # we weren't passed a tuple back, assume the user only
            # passed back something to the room which it was sent from
            returnRoom = ""
            returnVal = tempVal
