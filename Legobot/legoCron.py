...
def conv_to_set(obj, divisibleVal):  # Allow single integer to be provided
    ...
    try:
        ...
        if "/" in obj:
            ...
            if fIsNum(divisor):
                return [x for x in
                        range(divisibleVal) if x % int(divisor) == 0]
            else:
                return False
...
class Event(object):
    def __init__(self, func, min=allMatch, hour=allMatch,
        ...
        if not(self.mins and self.hours and self.days and
               self.months and self.dow and self.sec):
            self.goodCron = False
        else:
            self.goodCron = True
    ...
    def __matchtime__(self, t):
        """Return True if this event should trigger
...
def fIsNum(val):
    """
    Inputs:
        takes val which should either be string, int or float type,
        but can be anything

    Outputs:
        returns whether or not val was able to be successfully
        cast as an int

    Purpose:
        Used to determine if a value is an integer or not, especially
        when taking input from IRC
    """
...
if __name__ == '__main__':
    testCron = Event(printTest, min="*", hour="*", day="*",
                     month="*", dow="*", sec="1,5,10,30")
