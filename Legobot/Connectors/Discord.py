...
class Heartbeat(threading.Thread, object):
    def __init__(self, ws, interval):
        ...
        self.sequence = 0
        logger.debug("Heartbeat handler initialized with interval of {}".format(self.interval))
...
class DiscoBot(threading.Thread, object):
    ...
    def _parse_metadata(self, message):
        ...
        if 'channel_id' in message['d']:
            metadata['source_channel'] = message['d']['channel_id']  # this needs to resolve to a human-friendly string
        else:
